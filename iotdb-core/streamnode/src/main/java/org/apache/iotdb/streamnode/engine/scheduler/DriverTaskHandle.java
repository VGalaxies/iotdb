...
public class DriverTaskHandle {
  ...
  private final FairRoundRobinQueue driverTaskQueue;
  ...
  private final AtomicReference<Priority> priority =
      new AtomicReference<>(new Priority(System.nanoTime()));

  public DriverTaskHandle(
      int driverTaskHandleId, FairRoundRobinQueue driverTaskQueue, OptionalInt maxDriversPerTask) {
  ...
  public synchronized Priority updateScheduledTimeInNanos(long durationNanos) {
    scheduledTimeInNanos += durationNanos;
    Priority newPriority = driverTaskQueue.updatePriority(priority.get(), System.nanoTime());
