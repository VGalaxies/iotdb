...
/** The scheduler interface of {@link StreamDriverTask}. */
public interface ITaskScheduler {
   ...
   */
  boolean readyToRunning(StreamDriverTask task);
   ...
   * @param task the task to be switched.
   */
  void runningToReady(StreamDriverTask task);

  /**
   * Switch a task from {@link DriverTaskStatus#RUNNING} to {@link DriverTaskStatus#BLOCKED}, which
   * means the task is idle and waiting for external data.
   *
   * @param task the task to be switched.
   */
  void runningToBlocked(StreamDriverTask task);
   ...
   * @param task the task to be switched.
   */
  void blockedToReady(StreamDriverTask task);
   ...
   * @param task the task to be switched.
   */
  void toAborted(StreamDriverTask task);
}
