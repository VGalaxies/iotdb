...
public class PlaceholderReplacer {
  ...
  public String replace(String subQuery, WindowEvent event, PartitionKey partitionKey) {
    ...
    result = result.replace("${START_TIME}", String.valueOf(event.getStartTime()));
    result = result.replace("${END_TIME}", String.valueOf(event.getEndTime()));
    result = result.replace("${ROW_NUM}", String.valueOf(event.getRowCount()));
    // TODO: replace ${ROWS}, ${1}, ${2}, etc.
    return result;
  }
}
