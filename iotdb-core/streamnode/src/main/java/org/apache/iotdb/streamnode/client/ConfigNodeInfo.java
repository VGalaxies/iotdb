...
public class ConfigNodeInfo {
  ...
  SystemPropertiesHandler systemPropertiesHandler = StreamNodeSystemPropertiesHandler.getInstance();
  ...
  public void loadConfigNodeList() throws StartupException {
    long startTime = System.currentTimeMillis();
    // properties contain CONFIG_NODE_LIST only when start as Stream node
    configNodeInfoReadWriteLock.writeLock().lock();
    try {
      ...
      if (onlineConfigNodes.isEmpty()) {
        throw new StartupException(
            "Removing is only allowed in an environment where the streamnode has been successfully started. "
  ...
  private static class StreamNodeInfoHolder {
    private static ConfigNodeInfo INSTANCE = new ConfigNodeInfo();

    private StreamNodeInfoHolder() {
      // Empty constructor
    }
  }

  public static ConfigNodeInfo getInstance() {
    return StreamNodeInfoHolder.INSTANCE;
  }
}
