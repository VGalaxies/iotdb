...
/** Synchronously send RPC requests to StreamNodes. */
public class SyncStreamNodeClientPool {
  ...
  private final IClientManager<TEndPoint, SyncStreamNodeInternalServiceClient> clientManager;

  protected ImmutableMap<
          CnToSnSyncRequestType,
          CheckedBiFunction<Object, SyncStreamNodeInternalServiceClient, Object, Exception>>
      actionMap;

  private SyncStreamNodeClientPool() {
    clientManager =
        new IClientManager.Factory<TEndPoint, SyncStreamNodeInternalServiceClient>()
            .createClientManager(
                new ClientPoolFactory.SyncStreamNodeInternalServiceClientPoolFactory());
    buildActionMap();
  ...
  public Object sendSyncRequestToStreamNodeWithRetry(
    ...
    for (int retry = 0; retry < DEFAULT_RETRY_NUM; retry++) {
      try (SyncStreamNodeInternalServiceClient client = clientManager.borrowClient(endPoint)) {
  ...
  private Object executeSyncRequest(
      CnToSnSyncRequestType requestType, SyncStreamNodeInternalServiceClient client, Object req)
      throws Exception {
