...
@REM You can set StreamNode memory size, example '2G' or '2048M'
set MEMORY_SIZE=
...
@REM You need to change this IP as a public IP if you want to remotely connect IoTDB StreamNode by JMX.
@REM  0.0.0.0 is not allowed
set JMX_IP="127.0.0.1"

if %JMX_LOCAL% == "false" (
  echo "setting remote JMX..."
  set STREAMNODE_JMX_OPTS=-Dcom.sun.management.jmxremote^
  -Dcom.sun.management.jmxremote.port=%JMX_PORT%^
  -Dcom.sun.management.jmxremote.rmi.port=%JMX_PORT%^
  -Djava.rmi.server.randomIDs=true^
  -Dcom.sun.management.jmxremote.ssl=false^
  -Dcom.sun.management.jmxremote.authenticate=false^
  -Dcom.sun.management.jmxremote.password.file="%STREAMNODE_CONF%\jmx.password"^
  -Dcom.sun.management.jmxremote.access.file="%STREAMNODE_CONF%\jmx.access"^
  -Djava.rmi.server.hostname=%JMX_IP%
) else (
  echo "setting local JMX..."
)

set STREAMNODE_JMX_OPTS=%STREAMNODE_JMX_OPTS% -Diotdb.jmx.local=%JMX_LOCAL%
...
@REM StreamNode is a compute-only node, suggest using system memory 2 / 10
set /a suggest_=%system_memory_in_mb%/10*2
...
set STREAMNODE_HEAP_OPTS=-Xmx%ON_HEAP_MEMORY% -Xms%ON_HEAP_MEMORY%
set STREAMNODE_HEAP_OPTS=%STREAMNODE_HEAP_OPTS% -XX:MaxDirectMemorySize=%OFF_HEAP_MEMORY%
set STREAMNODE_HEAP_OPTS=%STREAMNODE_HEAP_OPTS% -Djdk.nio.maxCachedBufferSize=%MAX_CACHED_BUFFER_SIZE%
set STREAMNODE_HEAP_OPTS=%STREAMNODE_HEAP_OPTS% -XX:+CrashOnOutOfMemoryError
...
IF "%1" equ "printgc" (
	IF "%JAVA_VERSION%" == "8" (
	    md "%STREAMNODE_HOME%\logs"
		set STREAMNODE_HEAP_OPTS=%STREAMNODE_HEAP_OPTS% -Xloggc:"%STREAMNODE_HOME%\logs\gc.log" -XX:+PrintGCDateStamps -XX:+PrintGCDetails  -XX:+PrintGCApplicationStoppedTime -XX:+PrintPromotionFailure -XX:+UseGCLogFileRotation -XX:NumberOfGCLogFiles=10 -XX:GCLogFileSize=10M
	) ELSE (
		md "%STREAMNODE_HOME%\logs"
		set STREAMNODE_HEAP_OPTS=%STREAMNODE_HEAP_OPTS%  -Xlog:gc=info,heap*=trace,age*=debug,safepoint=info,promotion*=trace:file="%STREAMNODE_HOME%\logs\gc.log":time,uptime,pid,tid,level:filecount=10,filesize=10485760
	)
)
...
echo If you want to change this configuration, please check conf/windows/streamnode-env.bat.
